...
//   - parameter sender: a reference to the button that has been touched
  @IBAction func loginButtonClicked(_ sender: UIButton) {
    ...
    guard authenticationContext.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error1) else {
      ...
      reply: { [unowned self] (success, error) -> Void in
        ...
        if( success ) {
          ...
          if let error = error {
            ...
            let message = error.localizedDescription.description //self.errorMessageForLAErrorCode(errorCode: error.code)
